...
| File | Purpose |
|---|---|
| `playroom.config.js` | Entry point — components, frame, widths, `storageKey`, `exampleCode`, webpack overrides |
...
**Mantine `displayName` — fixed in Storybook, not in Playroom config** — Mantine components carry `displayName: '@mantine/core/ComponentName'`. `reactElementToJSXStringOptions` is NOT a supported option in Playroom v1 (`PlayroomConfig` type doesn't include it; Playroom's source doesn't consume it; functions are silently stripped by `JSON.stringify` in `DefinePlugin`). Do NOT add `reactElementToJSXStringOptions` to `playroom.config.js`. The fix lives in `.storybook/preview.tsx` — a `parameters.docs.source.transform` strips the `@scope/package/` prefix from all Storybook-generated code snippets so they are valid JSX when pasted into Playroom (see Storybook Config section).

### Session state

`storageKey: 'mantine-architect-v1'` is set explicitly in `playroom.config.js`. Playroom stores the code panel contents under this key in `localStorage`. Bump the version suffix (→ `v2`, etc.) to silently reset all stored sessions — for example after renaming a component that appears in saved snippets, or after a major scope change. Without an explicit key, Playroom auto-generates one from the project path; an explicit key ensures predictable resets.

`exampleCode` provides a clean starting template for new sessions (no stored state). It demonstrates `Text`, `ButtonMenu`, `Badge`, `Group`, and `Stack` without import statements.

### Storybook → Playroom copy flow

Storybook's "Show code" snippets are generated by `react-element-to-jsx-string`, which uses each component's `displayName` verbatim. Mantine components have `displayName: '@mantine/core/ComponentName'`, so the raw output is invalid JSX (e.g. `<@mantine/core/Text>`).

Fixed in `.storybook/preview.tsx` via `parameters.docs.source.transform`:
```ts
transform: (code: string) =>
  code.replace(/<(\/?|)@[\w-]+\/[\w-]+\//g, '<$1'),
```
This strips the `@scope/package/` prefix from every opening and closing tag in the generated snippet, producing valid JSX (`<Text>`, `<Group>`, etc.) that pastes directly into Playroom.

**Do NOT add `reactElementToJSXStringOptions` to `playroom.config.js`** — it is not a supported Playroom v1 config option and has no effect.
...
- Global `MantineProvider` with `theme={{ primaryShade: 8 }}` in `.storybook/preview.tsx`
...
- **`parameters.docs.source.transform`** strips `@mantine/core/` prefixes from generated code snippets — see Playroom → Storybook → Playroom copy flow section. Do not remove it.
